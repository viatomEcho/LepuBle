...
object LpWorkManager {
    ...
    /**
     * 开启扫描任务，做了1s延迟，防止频繁操作扫描
     */
    var startScan: Job? = null
    /**
     * 等待扫描结果（当status=6重新开启扫描），扫描没有结果返回时重启扫描
     */
    var isWaitingScanResult = false
    var scanTimer = object : CountDownTimer(3000, 3000) {
        override fun onTick(millisUntilFinished: Long) {
            LepuBleLog.d(tag, "scanTimer onTick, 等待扫描结果倒计时")
        }
        override fun onFinish() {
            LepuBleLog.d(tag, "scanTimer onFinish, 等待扫描结果倒计时结束, 重启扫描")
    ...
    private fun initBle() {
        ...
        if (leScanner == null) {
            LepuBleLog.d(tag, "initBle, leScanner is null")
        }
    ...
    fun initInterfaces(m: Int): BleInterface? {
        LepuBleLog.d(tag, "initInterfaces, vailFace.size:${vailFace.size()}, model:$m")
        ...
        when(m) {
            ...
            Bluetooth.MODEL_VTM01 -> {
                ...
                LepuBleLog.e(tag, "initInterfaces, 未识别此model:$m")
                return null
            }
        }
    }
    ...
    fun initManagers(m: Int, context: Context): LpBleManager? {
        LepuBleLog.d(tag, "initManagers, LpBleManager.size:${vailManager.size()}, model:$m")
        ...
        when(m) {
            ...
            Bluetooth.MODEL_BIOLAND_BGM -> {
                ...
                LepuBleLog.e(tag, "initManagers, 未识别此model:$m")
                return null
            }
        }
    }
    ...
    /**
     * 根据设备具体蓝牙名或者蓝牙地址进行扫描结果外发
     */
    fun setScanDefineDevice(isScanDefineDevice: Boolean, isScanByName: Boolean, defineDevice: String) {
        LepuBleLog.d(tag, "setScanDefineDevice, isScanDefineDevice:$isScanDefineDevice, " +
                "isScanByName:$isScanByName, defineDevice:$defineDevice")
    ...
    fun startDiscover(scanModel: IntArray? = null, needPair: Boolean = false, isReconnecting: Boolean = false) {
        if (vailFace.isEmpty() && isReconnecting) {
            LepuBleLog.d(tag, "startDiscover failed, vailFace.isEmpty and isReconnecting")
            return
        }
        stopDiscover()
        ...
        LepuBleLog.d(tag, "startDiscover, vailFace.size:${vailFace.size()}, needPair:$needPair, " +
                "isReconnecting:$isReconnecting, scanModel:${scanModel?.joinToString()}")
    }
     ...
     */
    fun stopDiscover() {
        LepuBleLog.d(tag, "into stopDiscover")
        startScan?.cancel()
        scanDevice(false)
    }
    ...
    fun reconnect(scanModel: IntArray? = null, reconnectDeviceName: Array<String>, needPair: Boolean = false, toConnectUpdater: Boolean = false) {
        if (vailFace.isEmpty()) {
            LepuBleLog.d(tag, "reconnect failed, vailFace.isEmpty")
            return
        }
        var reScan = false
        if (BleServiceHelper.BleServiceHelper.hasUnConnected()) {
            LepuBleLog.d(tag, "reconnectByName, 有未连接的设备需要重连")
            reScan = true
        }
        if (reScan) {
        ...
        LepuBleLog.d(tag, "reconnectByName, scanModel:${scanModel?.joinToString()}, reconnectDeviceName:${reconnectDeviceName.joinToString()}, " +
                "ReScan:$reScan, needPair:$needPair, toConnectUpdater:$toConnectUpdater")
    }
    ...
    fun reconnectByAddress(scanModel: IntArray? = null, reconnectDeviceAddress: Array<String>, needPair: Boolean = false, toConnectUpdater: Boolean = false) {
        if (vailFace.isEmpty()) {
            LepuBleLog.d(tag, "reconnectByAddress failed, vailFace.isEmpty")
            return
        }
        var reScan = false
        if (BleServiceHelper.BleServiceHelper.hasUnConnected()) {
            LepuBleLog.d(tag, "reconnectByAddress, 有未连接的设备需要重连")
            reScan = true
        }
        if (reScan) {
        ...
        LepuBleLog.d(tag, "reconnectByAddress, scanModel:${scanModel?.joinToString()}, reconnectDeviceAddress:${reconnectDeviceAddress.joinToString()}, " +
                "ReScan:$reScan, needPair:$needPair, toConnectUpdater:$toConnectUpdater")
    }
    ...
    private fun scanDevice(enable: Boolean) {
        LepuBleLog.d(tag, "scanDevice, enable:$enable, true(startScan) or false(stopScan), 等待扫描结果倒计时取消")
        scanTimer.cancel()
        if (enable) {
            if (bluetoothAdapter?.isEnabled!!) {
                ...
                if (leScanner == null) {
                    leScanner = bluetoothAdapter?.bluetoothLeScanner
                    LepuBleLog.d(tag, "scanDevice startScan, leScanner is null")
                }
                isDiscovery = true
                isWaitingScanResult = true
                leScanner?.startScan(null, settings, leScanCallback)
                scanTimer.start()
                LepuBleLog.d(tag, "scanDevice startScan, scanTimer.start等待扫描结果倒计时开始, isWaitingScanResult:$isWaitingScanResult")
            } else {
                LepuBleLog.d(tag, "scanDevice startScan, bluetoothAdapter.isEnabled is false")
            }
        } else {
            if (bluetoothAdapter?.isEnabled!!) {
                if (leScanner == null) {
                    leScanner = bluetoothAdapter?.bluetoothLeScanner
                    LepuBleLog.d(tag, "scanDevice stopScan, leScanner is null")
                }
                leScanner?.stopScan(leScanCallback)
            }
            isDiscovery = false
            isWaitingScanResult = false
            LepuBleLog.d(tag, "scanDevice stopScan, isWaitingScanResult:$isWaitingScanResult")
        }
    }
    ...
    private val leScanCallback: ScanCallback = object : ScanCallback() {
        override fun onScanResult(
            ...
            if (isWaitingScanResult) {
                isWaitingScanResult = false
                scanTimer.cancel()
                LepuBleLog.d(tag, "onScanResult, 扫描有结果返回, 等待扫描结果倒计时取消, isWaitingScanResult:$isWaitingScanResult")
            }
            ...
            if (model == Bluetooth.MODEL_UNRECOGNIZED) {
                LepuBleLog.d(tag, "onScanResult, 未识别此model:$model, needPair:$needPair, isScanDefineDevice:$isScanDefineDevice")
                if (scanModel == null) {
                    if (isScanDefineDevice) {
                        if (isScanByName) {
                            if (!deviceName.equals(scanByName)) {
                                LepuBleLog.d(tag, "onScanResult, scanModel is null, model:$model, 未扫描到指定蓝牙名的设备, " +
                                        "deviceName:$deviceName, scanByName:$scanByName")
                                return
                            }
                        } else {
                            if (!deviceAddress.equals(scanByAddress)) {
                                LepuBleLog.d(tag, "onScanResult, scanModel is null, model:$model, 未扫描到指定蓝牙地址的设备, " +
                                        "deviceAddress:$deviceAddress, scanByAddress:$scanByAddress")
                                return
                            }
                        }
                    }
                    if (needPair) {
                        ...
                        LepuBleLog.d(tag, "onScanResult, scanModel is null, 扫描结果需要配对, post to EventDeviceFound_ScanRecordUnRegister")
                    }
                    LiveEventBus.get<ScanResult>(EventMsgConst.Discovery.EventDeviceFoundForUnRegister).post(result)
                    LepuBleLog.d(tag, "onScanResult, scanModel is null, post to EventDeviceFoundForUnRegister")
                }
                return
            }
            val b = Bluetooth(
            ...
            if(scanModel != null) {
                if (!filterResult(b)) {
                    LepuBleLog.d(tag, "onScanResult filterResult, 未扫描到指定scanModel的设备")
                    return
                }
            }
            if (isScanDefineDevice) {
                if (isScanByName) {
                    if (!b.name.equals(scanByName)) {
                        LepuBleLog.d(tag, "onScanResult, model:$model, 未扫描到指定蓝牙名的设备, b.name:${b.name}, scanByName:$scanByName")
                        return
                    }
                } else {
                    if (!b.macAddr.equals(scanByAddress)) {
                        LepuBleLog.d(tag, "onScanResult, model:$model, 未扫描到指定蓝牙地址的设备, b.macAddr:${b.macAddr}, scanByName:$scanByAddress")
                        return
                    }
                }
            }
            if (needPair) {
                ...
                LepuBleLog.d(tag, "onScanResult, 扫描结果需要配对, post to EventDeviceFound_ScanRecord")
            }
            if (BluetoothController.addDevice(b)) {
                LepuBleLog.d(tag, "onScanResult, model = ${b.model}, isReconnecting::$isReconnectScan, b= ${b.name}, recName = ${reconnectDeviceName.joinToString()}, " +
                ...
                if (isReconnectScan && isContains) {
                    ...
                    if (model == Bluetooth.MODEL_AOJ20A || model == Bluetooth.MODEL_LPM311
                        ...
                        GlobalScope.launch {
                            ...
                            LepuBleLog.d(tag, "onScanResult, 发现需要重连的设备....去连接 model = ${b.model} name = ${b.name}  address = ${b.macAddr}")
                        }
                    } else {
                        vailFace.get(b.model)?.connect(application!!, b.device, true, toConnectUpdater)
                        LepuBleLog.d(tag, "onScanResult, 发现需要重连的设备....去连接 model = ${b.model} name = ${b.name}  address = ${b.macAddr}")
                    ...
                    // ER1进入升级后蓝牙名会改变，不在重连名单，直接外发给app处理
                    if (isReconnectScan) {
                        LepuBleLog.d(tag, "onScanResult, 找到了新蓝牙名设备，去连接Updater${b.name}")
        ...
        override fun onScanFailed(errorCode: Int) {
            LepuBleLog.e(tag, "onScanFailed, errorCode: $errorCode")
