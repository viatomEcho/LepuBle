...
public class LeBp2wBleCmd {
    ...
    public static byte[] getFileListCrc(int fileType, byte[] key) {
        switch (fileType) {
            case FileType.ECG_TYPE:
                return getReq(GET_ECG_LIST_CRC, new byte[0], key);
            case FileType.BP_TYPE:
                return getReq(GET_BP_LIST_CRC, new byte[0], key);
            case FileType.USER_TYPE:
                return getReq(GET_USER_LIST_CRC, new byte[0], key);
            default:
                return new byte[0];
        }
    }

    public static byte[] getWifiVersion(byte[] key) {
        return getReq(GET_WIFI_VERSION, new byte[0], key);
    }
