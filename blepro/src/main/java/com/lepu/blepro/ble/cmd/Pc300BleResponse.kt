...
//        var sign: Boolean
    ...
    class EcgResult(val bytes: ByteArray) {
        ...
        fun getMess(result: Int): String {
            return when(result) {
                0x00 -> "波形未见异常(60bpm <= HR <= 100 bpm, 且未发现任何心律不规则)"
                0x01 -> "波形疑似心跳稍快请注意休息(100bpm < HR <= 110 bpm)"
                0x02 -> "波形疑似心跳过快请注意休息(HR > 110 bpm 时)"
                0x03 -> "波形疑似阵发性心跳过快请咨询医生(连续3个以上的早搏)"
                0x04 -> "波形疑似心跳稍缓请注意休息(50 bpm <= HR < 60 bpm 时)"
                0x05 -> "波形疑似心跳过缓请注意休息(HR < 50 bpm 时)"
                0x06 -> "波形疑似偶发心跳间期缩短请咨询医生(类似于早搏)"
                0x07 -> "波形疑似心跳间期不规则请咨询医生(类似于二联律、三联律)"
                0x08 -> "波形疑似心跳稍快伴有偶发心跳间期缩短请咨询医生(早搏 + 心动稍快)"
                0x09 -> "波形疑似心跳稍缓伴有偶发心跳间期缩短请咨询医生(早搏 + 心动稍缓)"
                0x0A -> "波形疑似心跳稍缓伴有心跳间期不规则请咨询医生(心动稍缓 + 二联律/三联律)"
                0x0B -> "波形有漂移请重新测量(测量中有信号饱和或导联脱落，但还能测出平均心率)"
                0x0C -> "波形疑似心跳过快伴有波形漂移请咨询医生(心动过快 + 波形漂移)"
                0x0D -> "波形疑似心跳过缓伴有波形漂移请咨询医生(心动过缓 + 波形漂移)"
                0x0E -> "波形疑似偶发心跳间期缩短伴有波形漂移请咨询医生(早搏 + 波形漂移)"
                0x0F -> "波形疑似心跳间期不规则伴有波形漂移请咨询医生(二联律/三联律 + 波形漂移)"
                0xE0 -> "尚未进行过心电测试，无结果"
                0xFF -> "信号较差请重新测量(波形较小或基线漂移过大时, 已不能测出平均心率)"
                else -> ""
            }
        }
