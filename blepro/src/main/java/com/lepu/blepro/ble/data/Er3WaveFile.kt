...
class Er3WaveFile {
     ...
     * 根据每块8导联数据获取每导数据
     */
    fun getEachLeadData(oriInts: IntArray, leadName: String) : IntArray {
        val waveInts = mutableListOf<Int>()
        when (leadName) {
            "V6" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i])
                }
            }
            "I" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+1])
                }
            }
            "II" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+2])
                }
            }
            "V1" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+3])
                }
            }
            "V2" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+4])
                }
            }
            "V3" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+5])
                }
            }
            "V4" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+6])
                }
            }
            "V5" -> {
                for (i in oriInts.indices step 8) {
                    waveInts.add(oriInts[i+7])
                }
            }
            "III" -> {
                for (i in oriInts.indices step 8) {
                    waveInts[i.div(8)] = oriInts[i+2] - oriInts[i+1]
                }
            }
            "aVR" -> {
                for (i in oriInts.indices step 8) {
                    waveInts[i.div(8)] = -(oriInts[i+1] + oriInts[i+2]).div(2)
                }
            }
            "aVL" -> {
                for (i in oriInts.indices step 8) {
            ...
            "aVF" -> {
                for (i in oriInts.indices step 8) {
                    waveInts[i.div(8)] = oriInts[i+2] - oriInts[i+1].div(2)
                }
            }
        }
        return waveInts.toIntArray()
    }
