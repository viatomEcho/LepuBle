...
void SWT::SamplesToSwdSwa(double * samples, int sampleCnt)
{
    ...
    //the process of swt
    for (i = 0; i < SWT_FLOOR_NUM; i++)		// Panjie: 小波分解的层数，该算法采用3层小波分解，最后分解为D1，D2，D3，A3
    {
//        exponent2 = (int) pow(2, i);
        exponent2 = MyPow(2, i);
        exponent2Multi8 = FILTER_INI_LEN * exponent2;		// Panjie： 每一层分析时，滤波器的长度
...
//    int senarios;
    ...
    for (i = 0; i < SWT_FLOOR_NUM; i++)
    {
        for (j = 0; j < sampleCnt; j++)
        {
            absX = fabs(swd[i][j]);		// Panjie: 取细节系数绝对值
            arrayForOrder[j] = absX;
        }

        sort(arrayForOrder.begin(), arrayForOrder.end());		// Panjie：sort两个参数分别对应待排序数组的首地址和尾地址

        if(sampleCnt % 2 == 0)
        {
            lamda = (arrayForOrder[sampleCnt / 2-1] + arrayForOrder[sampleCnt / 2]) / 2;		// Panjie: 取中值
        }
        else
        {
...
//        senarios = GetSenarios(x1, x2, x3);
        ...
        {
            absX = fabs(swd[i][j]);
            if (-1 != DoubleCompares(absX, lamda) )		// Panjie: lamda为阈值，当absX大于等于lamda时保留absX
            {
                continue;
            }
            else
...
void SWT::SwdSwaToSamples(double * samples, int sampleCnt)
{
	...
	{
		samples[i] = swa[SWT_FLOOR_NUM - 1][i];		// Panjie: 取第三层的近似系数
	}
...
void SWT::EcgSwt(double * samples, int sampleCnt)
{
    ...
    if (sampleCnt > SWT_MAX_SAMPLE_CNT)		// Panjie: 一次分析长度只能是256
    {
        return;
    }
...
void SWT::AnalyzeSwt(deque <double> & samples)
{
    ...
    if (samples.size() != SWT_MAX_SAMPLE_CNT)		// Panjie : 样本个数再判断
    {
        return;
    }

    double * samplesPtr = new double[samples.size()];		// Panjie：申请内存空间
