...
public class MyLineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawTriangle(Canvas c, @Nullable ILineDataSet dataSet) {
        ...
        //     mRenderPaint.setColor(Color.parseColor("#cca3afb3"));
        ...
        if (e1 != null) {
            ...
            for (int x = count > 1 ? minx + 1 : minx; x < count; x++) {
                ...
                if (e1.getVal() != -1 && e2.getVal() != -1
                        && e1.getVal() != 0 && e2.getVal() != 0
                        && e1.getVal() != 65535 && e2.getVal() != 65535) {

                    LogUtils.d(e1.getVal() + "--e1--e2--" + e2.getVal());
...
//                    mRenderPaint.setColor(dataSet.getColor());
                ...
                Path path = new Path();
                for (int i = 0; i < mLineBuffer.length; ) {
                    path.reset();
                    path.moveTo(mLineBuffer[i] - 10, mLineBuffer[i + 1]);
                    path.lineTo(mLineBuffer[i], mLineBuffer[i + 1] - PhoneGlobal.VibratiobMarkHeight); //动态震动标记高度
                    path.lineTo(mLineBuffer[i] + 10, mLineBuffer[i + 1]);
                    path.close();
            ...
            if (e1 != null) {
                ...
                for (int x = count > 1 ? minx + 1 : minx; x < count; x++) {
                    ...
                    if (e1 == null || e2 == null) {
                      ...
                      if (e1.getVal()!=-1&&e2.getVal()!=-1&&e1.getVal()!=65535&&e2.getVal()!=65535){
                    //大于150的心率值显示在>=150的线上
                    if (e1.getVal() >= 150) {
                        LogUtils.d("hr value above 150 --val-- " + e1.getVal());
                        //e1.setData(String.valueOf(e1.getVal()));
                        LogUtils.d("hr value above 150 --data-- " + e1.getData().toString());
                        // e1.setVal(150);
                    }

                    if (e2.getVal() >= 150) {
                        // e2.setData(String.valueOf(e2.getVal()));
                        //  e2.setVal(150);
                    }
                    mLineBuffer[j++] = e1.getXIndex();
                    mLineBuffer[j++] = e1.getVal() * phaseY;
